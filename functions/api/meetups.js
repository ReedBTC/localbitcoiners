const UPSTREAM_URL = "https://relay.mynostr.app/meetups.json";

// Bound the upstream fetch: 10s wall-clock, 2 MB body cap. Same rationale
// as community-articles.js — an unbounded fetch could pin the Pages Function's
// CPU/memory budget. The cap is generous (current file is ~10 KB) but leaves
// headroom as boosted meetups/events accumulate.
const FETCH_TIMEOUT_MS = 10_000;
const RESPONSE_MAX_BYTES = 2 * 1024 * 1024; // 2 MB

// Exact-match origin allowlist — mirrors community-articles.js. `startsWith`
...
export async function onRequest(context) {
  ...
  try {
    const resp = await fetch(UPSTREAM_URL, {
      headers: { "User-Agent": "LocalBitcoiners-Meetups-Proxy/1.0" },
      cf: { cacheTtl: 300, cacheEverything: true },
      signal: ctrl.signal,
    });

    if (!resp.ok) {
      return new Response("Meetups upstream returned an error", {
        status: 502,
        headers: corsHeaders,
      });
    }
    ...
    if (Number.isFinite(cl) && cl > RESPONSE_MAX_BYTES) {
      return new Response("Upstream meetups file exceeded size limit", {
        status: 502,
        headers: corsHeaders,
      });
    }
    ...
    return new Response(
      isTimeout ? "Meetups upstream timed out" : "Failed to fetch meetups",
