const UPSTREAM_URL = "https://relay.mynostr.app/zaps.json";
...
// CPU/memory budget. The cap is generous (current file is ~55 KB) but leaves
// headroom as zaps accumulate.
...
export async function onRequest(context) {
  ...
  try {
    const resp = await fetch(UPSTREAM_URL, {
      headers: { "User-Agent": "LocalBitcoiners-Zaps-Proxy/1.0" },
      cf: { cacheTtl: 300, cacheEverything: true },
      signal: ctrl.signal,
    });

    if (!resp.ok) {
      return new Response("Zaps upstream returned an error", {
        status: 502,
        headers: corsHeaders,
      });
    }
    ...
    if (Number.isFinite(cl) && cl > RESPONSE_MAX_BYTES) {
      return new Response("Upstream zaps file exceeded size limit", {
        status: 502,
        headers: corsHeaders,
      });
    }
    ...
    return new Response(
      isTimeout ? "Zaps upstream timed out" : "Failed to fetch zaps",
