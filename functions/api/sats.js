const UPSTREAM_URL = "https://relay.mynostr.app/sats.json";
...
// CPU/memory budget. The cap is generous (current file is ~451 KB) but leaves
// headroom as episodes/boosts accumulate.
...
export async function onRequest(context) {
  ...
  try {
    const resp = await fetch(UPSTREAM_URL, {
      headers: { "User-Agent": "LocalBitcoiners-Sats-Proxy/1.0" },
      cf: { cacheTtl: 300, cacheEverything: true },
      signal: ctrl.signal,
    });

    if (!resp.ok) {
      return new Response("Sats upstream returned an error", {
        status: 502,
        headers: corsHeaders,
      });
    }
    ...
    if (Number.isFinite(cl) && cl > RESPONSE_MAX_BYTES) {
      return new Response("Upstream sats file exceeded size limit", {
        status: 502,
        headers: corsHeaders,
      });
    }
    ...
    return new Response(
      isTimeout ? "Sats upstream timed out" : "Failed to fetch sats",
