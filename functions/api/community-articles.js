const UPSTREAM_URL = "https://relay.mynostr.app/community_articles.json";
...
export async function onRequest(context) {
  ...
  try {
    const resp = await fetch(UPSTREAM_URL, {
      headers: { "User-Agent": "LocalBitcoiners-CommunityArticles-Proxy/1.0" },
      cf: { cacheTtl: 300, cacheEverything: true },
      signal: ctrl.signal,
    });

    if (!resp.ok) {
      return new Response("Community articles upstream returned an error", {
        status: 502,
        headers: corsHeaders,
      });
    }
    ...
    if (Number.isFinite(cl) && cl > RESPONSE_MAX_BYTES) {
      return new Response("Upstream community articles file exceeded size limit", {
        status: 502,
        headers: corsHeaders,
      });
    }
    ...
    return new Response(
      isTimeout ? "Community articles upstream timed out" : "Failed to fetch community articles",
